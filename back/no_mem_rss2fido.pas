...
uses crt, dos, utf_dos, rain, skMHL, skmhlsq, skmhljam, skmhlmsg, skCommon;
  ...
  Type tpar_cfg = Array[1..2] of Array[1..30] of Array[1..2] of string;
     ppar_cfg = ^tpar_cfg;
  var par_cfg : ppar_cfg;

  Type TChar = Array[0..1] of Char;
     PChar = ^TChar;
  var ch_rss, ch_out : PChar;
...
g_bool_exit:
  ...
  begin
    ...
    num_cfg_name_par := 1; num_set := 0; first := true;
      ...
      if (first) then // при первом идентификаторе группы default не работает
       begin
        ...
        begin
          ...
          //  size_par_cfg := size_par_cfg + sizeof(string);
...
begin
  ...
  p_help_par;
  size_par_cfg := sizeof(string)*5*30*5;
  getmem(par_cfg,size_par_cfg);
  {$I rss2fido.inc}
  str_sim_par_cfg := '-c';
  p_parse_str_in_prog;
...
// end load RS mem
  ...
  begin
    ...
    begin
      p_lang ('news', 1);
      getmem(ch_out,sizeof(char)*num_end_rss);  // num_end_rss -- а сколько?
