...
uses crt, dos, utf_dos, rain, skMHL, skmhlsq, skmhljam, skmhlmsg, skCommon;
  ...
  Type ta1=Array[1..255] of Array[1..20] of string;
     pa1=^ta1;
  var par_cfg : pa1;
...
g_bool_exit:
  ...
  function f_pos_file_news(str_verif : string; temp_num_dec : byte) : boolean;
  var
    temp_nsvs : word;
  ...
  begin
    ...
    begin
      Seek (file_news, temp_num);
      read (file_news, fb);
      temp_str_verif := temp_str_verif + Chr(fb);
      inc (temp_num);
    end;
    temp_num := temp_nsvs;
    temp_num := temp_num - temp_num_dec;
  ...
  function f_rss_start (start_rss, end_rss : string) : boolean;

  begin
    while (f_pos_file_news (start_rss, 0) <> true) and (f_pos_file_news (end_rss, 0) <> true) and (temp_num < num_size) do
    begin
      inc (temp_num);
    end;
    if (f_pos_file_news (end_rss, 0) = true) or (temp_num >= num_size) then
    begin
      temp_num := num_size;
      f_rss_start := true;
    end
    ...
    rss_start := temp_num + length (start_rss);
  end;
  ...
  begin
    ...
    while (f_pos_file_news (str_1, 0) <> true) and (f_pos_file_news ('</item>', 0) <> true) and (temp_num < num_size) do
      inc (temp_num);

    if (f_pos_file_news (str_1, 0) = true) and (temp_num < num_size) then
    begin
      while (f_pos_file_news (str_2, 0) <> true) and (f_pos_file_news ('</item>', 0) <> true) and (temp_num < num_size) do
      begin
        inc (temp_num);
      end;
      if (f_pos_file_news (str_2, 0) = true) or (temp_num >= num_size) then
      begin
        f_pnaddapp2 := true;
        exit;
      end;
  ...
  begin
    ...
    temp_num := rss_start;
    while (not f_pos_file_news (str_pnaa1, 0)) and (temp_num < num_size) do
      inc (temp_num);
    if (temp_num >= num_size) then
      exit;
    p_lang('news_add');

    num_str := 0;
    temp_num := temp_num + length (str_pnaa1);
    while (not f_pos_file_news (str_pnaa2, 0)) do
    begin
      inc(num_str);
      ...
      begin
        ...
        begin
          Seek (file_news, temp_num);
          read (file_news, fb);
          load_str[num_str] := load_str[num_str] + chr(fb);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            load_str[num_str] := f_string_fix(Copy(load_str[num_str], 1, num_str_del -1), mega_num_tpl_fix -1) + str_end_wr;
          end;
        end;
        inc(num_str);
      end;
    ...
    begin
      ...
      if (str_end_wr <> '') then
        load_str[1] := f_string_fix(Copy(load_str[1], 1, length(load_str[1]) - length(str_end_wr) -1), mega_num_tpl_fix -1) + str_end_wr;
  ...
  begin
    ...
    num_t := 1;
    while load_tpl[num_t] <> '' do
    begin
      if (pos('@', (load_tpl[num_t])) <> 0) and
         (pos(',[', (load_tpl[num_t])) <> 0) and
         (pos(']', (load_tpl[num_t])) <> 0) and
         (pos(',', (load_tpl[num_t])) <> 0) then
      begin
        str_teg := Copy(load_tpl[num_t], pos('@', (load_tpl[num_t])) +1, length(load_tpl[num_t]) - (pos('@', (load_tpl[num_t]))));
        str_teg := Copy(str_teg, 1, pos(',', str_teg) -1);

        mega_num_tpl_in_fix := pos(UpCase('@' + str_teg + ','), UpCase(load_tpl[num_t])) -1;
        num_re_str := f_str2num(Copy(load_tpl[num_t], pos(UpCase('@' + str_teg + ','), UpCase(load_tpl[num_t])) + length('@' + str_teg + ','), 1));
        tpl_tstr := Copy(load_tpl[num_t], pos(UpCase('@' + str_teg + ','), UpCase(load_tpl[num_t])) + length('@' + str_teg + ',') + 3, length(load_tpl) - (pos(UpCase('@' + str_teg + ','), UpCase(load_tpl[num_t])) + length('@' + str_teg + ',') + 3));
        ...
        str_start_wr := Copy(load_tpl[num_t], 1, mega_num_tpl_in_fix);
        str_end_wr := Copy(load_tpl[num_t], pos(UpCase('@' + str_teg + ','), UpCase(load_tpl[num_t])) + length('@' + str_teg + ',') + 1 + length(tpl_tstr) + 3, length(load_tpl[num_t]) - ( pos(UpCase('@' + str_teg + ','), UpCase(load_tpl[num_t])) + length('@' + str_teg + ',') + length(tpl_tstr) + 3));
      ...
      p_write_add(UpCase(load_tpl[num_t]));
      inc(num_t);
    end;
  ...
  begin
    ...
    rewrite (file_out);
    while (temp_num < num_size) and (num_msg_t < num_msg) do
    begin
      p_news_add;
      inc(num_msg_t);
    end;
  ...
  function auto_code(put_file_auto_code : string) : string;
  var
    file_auto_code : file of byte;
    num_ac_t : longint;
    num_ac_t_c : longint;
    byte_ac : byte;
    srt_ac_tc : string;
  ...
  begin
    Assign(file_auto_code, put_file_auto_code);
    reset(file_auto_code);
    num_ac_t := 0; bool_code_yes := false;
    while (num_ac_t + length('encoding=') +1 < FileSize(file_auto_code)) and (bool_code_yes = false) do
    begin
      num_ac_t_c := 1; srt_ac_tc := '';
      while num_ac_t_c <= length('encoding=') +1 do
      begin
        seek(file_auto_code, num_ac_t + num_ac_t_c);
        read(file_auto_code, byte_ac);
        srt_ac_tc := srt_ac_tc + chr(byte_ac);
        inc(num_ac_t_c);
      end;
      ...
      begin
        ...
        num_ac_t_c := num_ac_t;
        repeat
          seek(file_auto_code, num_ac_t_c);
          read(file_auto_code, byte_ac);
          inc(num_ac_t_c);
        until (byte_ac = ord(#39)) or (byte_ac = ord(#34));
        srt_ac_tc := '';
        while num_ac_t < num_ac_t_c -1 do
        begin
          seek(file_auto_code, num_ac_t);
          read(file_auto_code, byte_ac);
          srt_ac_tc := srt_ac_tc + chr(byte_ac);
          inc(num_ac_t);
        end;
...
{$I+}
 ...
 // close(file_tpl);
  ...
  begin
    ...
    begin
      num_size_next := f_file_size(rss_name);
  ...
  begin
    reset (file_news);
    num_size := fileSize(file_news);
    temp_num := 1;
    while (f_pos_file_news ('<item>', 0) <> true) and
          (temp_num < num_size - length('<item>')) do
      inc (temp_num);
    if not ((temp_num < num_size - length('<item>')) and
       (temp_num < num_size)) then
    begin
      p_lang('error_item');
      item := false;
    end else
...
repeat
 ...
 // if not debage then
    comv_sw := auto_code(rss_name);
  p_lang('code');
  if (comv_sw = 'UTF-8') then
    file_convert_utf2dos(rss_name);
  if (comv_sw = 'WINDOWS-1251') or
     (comv_sw = 'NONE_CODE') then
    file_convert_win2dos(rss_name);
